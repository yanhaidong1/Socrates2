...
def main(argv=None):
    ...
    if open_markerUMAP_final == 'yes':
        ...
        if re.match('.+/(.+)\.atac\.soc\.qs2', input_soc_obj_fl):
            mt = re.match('.+/(.+)\.atac\.soc\.qs2', input_soc_obj_fl)
    ...
    if open_vis_marker_dotplot_final == 'yes':
        ...
        if open_only_dotplot_final == 'no':
            ...
            #ipt_prepared_gene_fl = output_dir + '/open_vis_marker_dotplot_final_dir/opt_gene_zscore_accprop.qs2'
            ipt_marker_fl = args.marker_file

            input_soc_obj_fl = open_vis_marker_dotplot_final_dir + '/' + input_prefix + '.atac.soc.qs2'
