...
##updaating 102824
##prepare an obj to store the sparse bed and gff file
...
###################################################################################################
#' updating 102724 
#' impute the gene accessibility
#' 
#' check if 'Clusters' is the target cluster
#' 
imputeGeneActivity <- function(obj_gba,
                               obj_cluster,
                               marker_fl,
                               output_dir,
                               rdType = 'NMF',
  ...
  ###################
  # modify input data
  activity <- obj_gba$scaled_gene_activity
  ...
  ##load meta
  if (rdType == 'NMF'){
    b <- obj_cluster$Clusters_NMF
  }
  if (rdType == 'SVD') {
    b <- obj_cluster$Clusters_SVD
  }
  
  ##validate the target cluster of b
  ...
  b <- b[!b[,target_cluster] %in% rmcluster,]
  
  ##load pcs
  if (do_harmony == 'yes'){
    h.pcs <- obj_cluster$HM_EMBS
  }else{
    h.pcs <- obj_cluster$PCA
  }
  
  ##filter the cells
  shared_cells <- intersect(colnames(activity),rownames(b))
  shared_cells <- intersect(shared_cells, rownames(h.pcs))
  
  activity <- activity[,shared_cells]
  b <- b[colnames(activity),]
  h.pcs <- h.pcs[colnames(activity),]
  
  ##add the marker information
  marker.info.dat <- read.delim(marker_fl,header=T)
  rownames(marker.info.dat) <- marker.info.dat$geneID
  
  ##run the smooth function
  out <- runMajorPriori(b, activity, h.pcs, marker.info.dat,output_dir,onlysmooth_marker,threads=threads,
                        impute_k=impute_k_val, 
                        impute_step=impute_step_val)
