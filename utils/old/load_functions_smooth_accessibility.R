...
runMajorPriori     <- function(all.b, 
    ...
    ##updating we need to first make sure marker.info is not NULL if we only want smooth the markers
    if (onlysmooth_marker == 'yes'){
      ##make sure the marker.info is existing
      if (!is.null(marker.info)){
        activity <- activity[rownames(activity) %in% as.character(marker.info$geneID),]
      }else{
        stop("ERROR: must provide marker file when smoothing the marker genes")
      }
    }
    #if(smooth.markers){
    #    activity <- activity[rownames(activity) %in% as.character(marker.info$geneID),]
    ...
    #We do not plot the UMAP right now
    ...
    #}
    # return
    #return(list(meta=all.b, activity=activity, impute.activity=impute.activity))
    ##only return the imputed activity
    return(list(meta=all.b, impute.activity=impute.activity, marker_infor = marker.info))
