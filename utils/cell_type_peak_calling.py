...
##udpating 120825 we will set an option to build the group bigwig building
...
from input_other_required_scripts_dir.utils_cell_type_peak_calling_dir_python import step05_build_bigwig_group_file as s5_bigwig
...
def get_parsed_args():
    ...
    ##updating 051925
    parser.add_argument("-open_build_gp_bw", dest = 'build_group_bigwig', help = 'Users choose to build the group bigwig.'
                                                                            'Default: no')

    parser.add_argument("-gp_meta_fl", dest='group_meta_file',
                        help='Users need to provide a meta file with group column name.')

    ##if we have the meta file we do not need the group file
    #parser.add_argument("-gp_fl", dest='group_file',
    #                    help='Users need to make a group patterns in a file. If -s3_open_diff_gp_peak is initiated, this argument need to be added. Otherwise,'
...
def main(argv=None):
    ...
    ##updating 120925
    if args.build_group_bigwig is None:

        open_build_group_bigwig_final = 'no'
        ...
        if args.build_group_bigwig != 'yes':
            print('Please specify the -build_group_bigwig yes')
            return
        else:
            open_build_group_bigwig_final = 'yes'

        if args.group_meta_file is None:
            print('Cannot find group meta file, please provide it')
            return
        else:
            try:
                file = open(args.group_meta_file, 'r')  ##check if the file is not the right file
            except IOError:
                print('There was an error opening the group meta file!')
                return
    ...
    if args.open_build_object_file is not None:
        ...
        if args.open_build_object_file != 'yes':
            print('Please specify the -build_gp_bw yes')
            return
        else:
    ...
    ##updating 120825
    if open_build_group_bigwig_final == 'no':
        ...
        s5_open_bigwig_build_group_final_dir = output_dir + '/s5_open_bigwig_build_group_final_dir'
        if not os.path.exists(s5_open_bigwig_build_group_final_dir):
            os.makedirs(s5_open_bigwig_build_group_final_dir)

        input_ref_fai_fl = args.genome_fai_file
        ipt_meta_fl = args.group_meta_file

        ##build the bed file
        shiftsize = '-75'
        extsize = '150'
        g_size = args.genome_size
        s5_bigwig.build_bigwig_fl_parallele (output_dir,s5_open_bigwig_build_group_final_dir,utils_cell_type_peak_calling_dir,input_ref_fai_fl,ipt_meta_fl,core_number_run,
                               g_size, extsize, shiftsize, final_max_gap_val, final_SLOCAL_val, final_LLOCAL_val)
