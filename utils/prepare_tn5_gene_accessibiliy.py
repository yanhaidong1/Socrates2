...
def get_parsed_args():
    ...
    parser.add_argument("-Ggff_fl", dest='gene_gff_file',
                        help='Provide the gene gff file used to extract exon region when permutating peaks.')
...
def main(argv=None):
    ...
    if args.s3_open_gene_accessibility is None:
        ...
        if args.s3_open_gene_accessibility == 'yes':
            ...
            ##input gene tn5 sparse file
            if args.gene_tn5_file is None:
                print('Cannot find gene tn5 file, please provide it')
                return
            else:
                try:
                    file = open(args.gene_tn5_file, 'r')  ##check if the file is not the right file
                except IOError:
                    print('There was an error opening the gene tn5 file!')
                    return

            ##input gene bed file
            if args.gene_bed_file is None:
                print('Cannot find gene bed file, please provide it')
                return
            else:
                try:
                    file = open(args.gene_bed_file, 'r')  ##check if the file is not the right file
                except IOError:
                    print('There was an error opening the gene bed file!')
                    return
    ...
    if args.s4_open_smooth_gene_accessibility is None:
        ...
        if args.s4_open_smooth_gene_accessibility == 'yes':
            ...
            ##input gene accessibility file
            if args.gene_accessibility_file is None:
                print('Cannot find gene accessibility file, please provide it')
                return
            else:
                try:
                    file = open(args.gene_accessibility_file, 'r')  ##check if the file is not the right file
                except IOError:
    ...
    if s2_open_prepare_gene_tn5_final == 'yes':
        ...
        s2_geneTn5.prepare_gene_tn5 (input_required_scripts_dir,input_tn5_bed_fl,s2_open_prepare_gene_tn5_final_dir,
                      ...
                      input_genome_fl,final_category_gff)
