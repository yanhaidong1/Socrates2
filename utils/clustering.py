...
def get_parsed_args():
    ...
    ##updating 091326
    ##remove the clusters flagged as low quality and re-cluster from the merged object
    parser.add_argument('-remove_clusters', dest = 'remove_clusters', help = 'Remove the given clusters and re-run the clustering from the merged object. '
                                                                             'The cluster labels are read from the <prefix>.atac.soc.qs2 produced by a previous run '
                                                                             'with the SAME -o, -prefix and parameters. Provide comma-separated cluster numbers, '
                                                                             'e.g. -remove_clusters "1,2". Cannot be combined with -only_cluster yes. '
                                                                             'Default: not used (no cluster removed)')
...
def main(argv=None):
    ...
    ##updating 091326
    ##remove low quality clusters, then re-cluster from the merged object
    remove_clusters_final = 'no'
    remove_clusters_suffix = ''
    remove_clusters_ref_final = ''

    if args.remove_clusters is not None:
        raw_value = re.sub(r'\s+', '', str(args.remove_clusters))
        tokens = [t for t in raw_value.split(',') if t != '']
        if len(tokens) == 0 or not all(re.match(r'^\d+$', t) for t in tokens):
            print('Please provide -remove_clusters as comma-separated cluster numbers, e.g. -remove_clusters "1,2"')
            return
        ##canonicalise: "01" -> "1", de-duplicate, sort numerically (keeps the cache suffix stable)
        tokens = sorted(set(str(int(t)) for t in tokens), key=int)

        if only_cluster == 'yes':
            print('Cannot combine -remove_clusters with -only_cluster yes: removing clusters requires re-running '
                  'the dimension reduction and the clustering from the merged object.')
            return

        ref_fl = output_dir + '/' + prefix_name_final + '.atac.soc.qs2'
        if not os.path.exists(ref_fl):
            print('Cannot find the cluster labels from a previous run: ' + ref_fl)
            print('Please run the clustering once (same -o, same -prefix, same parameters) before using -remove_clusters.')
            return

        ##snapshot the original labels so that repeated removals always refer to the same cluster numbering
        snap_fl = output_dir + '/' + prefix_name_final + '.atac.soc.refsnapshot.qs2'
        if not os.path.exists(snap_fl):
            shutil.copy2(ref_fl, snap_fl)
            print('Snapshot of the reference cluster labels created: ' + snap_fl)

        remove_clusters_final = ','.join(tokens)
        remove_clusters_suffix = '_rmC' + '-'.join(tokens)
        remove_clusters_ref_final = snap_fl

    store_final_parameter_line_list.append('remove_clusters_final <- ' + '\'' + remove_clusters_final + '\'')
    store_final_parameter_line_list.append('remove_clusters_suffix <- ' + '\'' + remove_clusters_suffix + '\'')
    store_final_parameter_line_list.append('remove_clusters_ref_final <- ' + '\'' + remove_clusters_ref_final + '\'')
    ...
    if return_code != 0:
        print('Clustering step failed with exit code ' + str(return_code) + ', stopping before the plotting step.')
        return
    ...
    cmd = 'Rscript ' + ipt_script + \
          ' ' + output_dir + '/' + prefix_name_final + '.atac.soc.qs2' + \
          ' ' + output_dir + \
          ' ' + prefix_name_final + \
          ' ' + ipt_config_fl
    print(cmd)
    return_code = subprocess.call(cmd,shell=True)
    if return_code != 0:
        print('Plotting step failed with exit code ' + str(return_code) + '.')
        return
