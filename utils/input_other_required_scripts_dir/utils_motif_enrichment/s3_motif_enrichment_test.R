...
peak_by_motif_rds_fl <- paste0(ipt_all_rds_dir,'/','opt_all_motif_matches_mtx.qs2')
#peak_by_motif_rds_fl_list <- list.files(path = ipt_all_rds_dir,pattern = '*motif_matches_mtx.qs2')

if (file.exists(paste0(input_output_dir,'/opt_final_motif_by_cell_mtx.qs2'))){
  
  ##updating 121622
  message (' - opt_motif_by_cell_mtx.qs2 is existed, and we will directly read it')
  motif_by_cell_mtx <- qs2::qs_read(paste0(input_output_dir,'/opt_final_motif_by_cell_mtx.qs2'))
  ...
  outs <- lapply(peak_by_cell_rds_fl_list,function(x){
    ...
    ipt_peak_by_cell_mtx = qs2::qs_read(paste0(ipt_all_rds_dir,'/',x))
    organnm_1 <- gsub('opt_','',x)
    organnm_2 <- gsub('_peak_cell_mtx.qs2','',organnm_1)
    ipt_peak_by_motif_mtx = qs2::qs_read(peak_by_motif_rds_fl)
    ...
    qs2::qs_save(motif_by_cell_mtx,paste0(input_output_dir,'/opt_',organnm_2,'_motif_by_cell_mtx.qs2'))
    
    return(motif_by_cell_mtx)
  })
  
  outs_final_combine <- do.call(cbind, outs)
  qs2::qs_save(outs_final_combine,paste0(input_output_dir,'/opt_final_motif_by_cell_mtx.qs2'))
  motif_by_cell_mtx <-  qs2::qs_read(paste0(input_output_dir,'/opt_final_motif_by_cell_mtx.qs2'))
...
#ipt_peak_by_cell_mtx <- qs2::qs_read(ipt_peak_by_cell_mtx_rds_fl)
#ipt_peak_by_motif_mtx <- qs2::qs_read(ipt_peak_by_motif_mtx_rds_fl)
...
#ipt_peak_by_cell_mtx <- ipt_peak_by_cell_mtx[shared_peaks,shared_cells]
#motif_by_cell_mtx <- ipt_motif_by_peak_mtx %*% ipt_peak_by_cell_mtx
#qs2::qs_save(motif_by_cell_mtx,paste0(input_output_dir,'/opt_motif_by_cell_mtx.qs2'))
...
qs2::qs_save(outs,paste0(input_output_dir,'/opt_outs.qs2'))
