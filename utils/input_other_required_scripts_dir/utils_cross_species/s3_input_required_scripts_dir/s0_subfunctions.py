...
##Note in the following step, we only keep the one gene pair in the syntenic region, the regions containing three or more will be filtred





def subfunction_check_syntenic_blocks (ipt_spe1_all_os_ACRs_fl, ipt_spe2_os_ACRs_fl,input_spe1_acr_fl,opt_dir,spe1_prefix,spe2_prefix):
    ...
    for eachsyntenic_region_ID in store_region_ID_gene_pair_sted_dic:
        ...
        ################################
        ##Here we check the gene length and only keep the gene length is 2 for the one syntenic region
        ###############################
    ...
    cmd = 'cut -f 1-3 ' + input_spe1_acr_fl + ' > ' + opt_dir + '/temp_' + spe1_prefix + '_acr.txt'
    print(cmd)
    subprocess.call(cmd,shell=True)
                ...
                if not col[0].startswith('ChrMt') and not col[0].startswith('ChrPt'):
                    ...
                    if 'scaffold' not in col[0]:
                        if 'Gm' in col[0]:
                            ...
                            if 'gene' in col[2] and 'ChrSy' not in col[0] and 'ChrUn' not in col[0]:
                                final_line = col[0] + '\t' + col[3] + '\t' + col[4] + '\t' + col[0] + '_' + col[
                                    3] + '_' + \
                                             col[4]
                                store_gene_bed_line_list.append(final_line)
