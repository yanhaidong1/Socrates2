...
def subfunction_summarize_overview (ipt_final_summary_blast_fl,ipt_sep2_syntenic_region_fl,input_rice_phy_score_fl,opt_dir,
                                    ipt_spe1_ACR_gene_Cate_fl,ipt_spe2_ACR_gene_Cate_fl,
...
##updating 111723
def subfunction_check_acc_in_rice_atlas_peaks (ipt_rice_atlas_peak_coverage_fl,ipt_total_rice_atlas_acr_fl,ipt_rice_leaf_syntenic_fl,
                                               opt_dir,
                                               spe1_prefix,spe2_prefix,
    ...
    store_rice_leaf_acr_dic = {}
    count = 0
    with open (ipt_rice_leaf_syntenic_fl,'r') as ipt:
        for eachline in ipt:
            eachline = eachline.strip('\n')
            col = eachline.strip().split()
            count += 1
            if count != 1:
                riceACR = col[1]
                rice_ACR_line = '\t'.join(col[1].split('_'))
                store_rice_leaf_acr_dic[rice_ACR_line] = 1

    with open (opt_dir + '/temp_' + spe1_prefix + '_acr_line.txt','w+') as opt:
        for eachacrline in store_rice_leaf_acr_dic:
            opt.write(eachacrline + '\n')

    cmd = 'sort -k1,1V -k2,2n ' + opt_dir + '/temp_' + spe1_prefix + '_acr_line.txt > ' + \
          opt_dir + '/temp_' + spe1_prefix + '_acr_line_sorted.txt'
    print(cmd)
    subprocess.call(cmd,shell=True)

    cmd = 'bedtools intersect -wa -wb -a ' + opt_dir + '/temp_' + spe1_prefix + '_acr_line_sorted.txt' + \
          ' -b ' + ipt_total_rice_atlas_acr_fl + ' > ' + opt_dir + '/temp_intersect_' + spe1_prefix + '_leafACR_to_atlas.txt'
    print(cmd)
    subprocess.call(cmd,shell=True)

    store_leaf_rice_ACR_atlas_ACR_dic = {}
    with open (opt_dir + '/temp_intersect_' + spe1_prefix + '_leafACR_to_atlas.txt','r') as ipt:
        for eachline in ipt:
            eachline = eachline.strip('\n')
            col = eachline.strip().split()
            leaf_acr = col[0] + '_' + col[1] + '_' + col[2]
            atlas_acr = col[3] + '_' + col[4] + '_' + col[5]
            if leaf_acr in store_leaf_rice_ACR_atlas_ACR_dic:
                store_leaf_rice_ACR_atlas_ACR_dic[leaf_acr][atlas_acr] = 1
            else:
                store_leaf_rice_ACR_atlas_ACR_dic[leaf_acr] = {}
                store_leaf_rice_ACR_atlas_ACR_dic[leaf_acr][atlas_acr] = 1


    ##The goal is to check distribution of number of cleltype where acr is open for both three categories
    ##we need to add the background
    store_final_line_list = []
    count = 0
    with open(ipt_rice_leaf_syntenic_fl, 'r') as ipt:
        for eachline in ipt:
            ...
            if count != 1:
                ...
                if rice_leaf_ACR in store_leaf_rice_ACR_atlas_ACR_dic:

                    atlas_acr_dic = store_leaf_rice_ACR_atlas_ACR_dic[rice_leaf_ACR]

                    for eachatlasacr in atlas_acr_dic:

                        celltypenum = store_eachpeak_celltype_str_num_dic[eachatlasacr]['celltypenum']

                        final_line = eachline + '\t' + eachatlasacr + '\t' + celltypenum
                        store_final_line_list.append(final_line)
                ...
                final_line = eachline + '\t' + 'atlasACR' + '\t' + 'atlasAccCelltypeNum'
                store_final_line_list.append(final_line)
