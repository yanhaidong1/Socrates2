...
def subfunction_iden_correspond (spe1_spe2_blast_fl,spe1_spe2_ref_blast_fl,
                                 spe1_acr_fl,spe1_spe2_syntenic_acr_fl,opt_dir,input_spe2_acr_fl,spe1_prefix,spe2_prefix):

    cmd = 'cut -f 1-3 ' + input_spe2_acr_fl + ' > ' + opt_dir + '/temp_' + spe2_prefix + '_acr.txt'
    print(cmd)
    ...
    store_final_line_list = []
    first_line = spe1_prefix + '_ACRID' + '\t' + spe1_prefix + '_ACR' + '\t' + \
                 spe2_prefix + '_region' + '\t' + spe2_prefix + '_ACR'
        ...
        for eachspe2region in spe2_region_dic:
            ...
            if eachspe2region in store_spe2_region_acr_dic:
                ...
                for eachspe2ACR in spe2_acr_dic:
                    ...
                    final_line = eachspe1ACRID + '\t' + eachspe1ACR + '\t' + \
                                 eachspe2region + '\t' + eachspe2ACR
                    store_final_line_list.append(final_line)
                ...
                final_line = eachspe1ACRID + '\t' + eachspe1ACR + '\t' + \
                             eachspe2region + '\t' + spe2_acr
                store_final_line_list.append(final_line)

    with open (opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_ACRblastedRegion.txt','w+') as opt:
        for eachline in store_final_line_list:
            opt.write(eachline + '\n')
    ...
    count = 0
    with open (opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_ACRblastedRegion.txt','r') as ipt:
        for eachline in ipt:
            eachline = eachline.strip('\n')
            col = eachline.strip().split()
            count += 1
    ...
    for eachACR in store_spe1_ACR_syntenic_region_dic:
        if eachACR not in store_ACR_blast_region_in_syntenic_dic:
            ...
            final_line = ACRID + '\t' + eachACR + '\t' + \
                         'none' + '\t' + 'none' + '\t' + celltype_cate + '\t' + syntenic_regionID_str
            store_final_line_list.append(final_line)

    with open (opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion.txt','w+') as opt:
        for eachline in store_final_line_list:
            opt.write(eachline + '\n')
...
def subfunction_check_syntenic_region_gene_pair_direction (input_spe1_gff_fl,input_spe2_gff_fl,
                        ...
                        if spe2_prefix == 'Uf':
                            ...
                            if spe2_prefix == 'Os':
                                ...
                                if spe2_prefix == 'Pm':
                                    ...
                                    ed = col[4]
                                    dir = col[6]
                                    store_spe2_gene_st_ed_dir_dic[geneID] = {'chrnm': chrnm, 'st': st, 'ed': ed,
                                                                             'dir': dir}
    ...
    #print(store_spe2_gene_st_ed_dir_dic)

    #if spe2_prefix == 'Pm':
    #    print('print Pm_gene')
    #    print(store_spe2_gene_st_ed_dir_dic['RLM58178.1'])
    #    print('############')
    ...
    with open (input_spe1_syntenic_genes_all_os_ACRs_fl,'r') as ipt:
        for eachline in ipt:
            ...
            ##updating 073025
            ##we will only consider the speceis region ID
            mt = re.match('(.+)_syntenic.+',syntenic_region_ID)
            spenm = mt.group(1)

            if spenm == spe2_prefix:
    ...
    with open (input_summary_fl,'r') as ipt:
        for eachline in ipt:
            ...
            if count != 1:
                ...
                if col[5] != 'NotMoreThanTwoGenesInPair':

                    syntenic_region_list = col[5].split(',')
                    genePair_dir_list = []
    ...
    with open(opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion_addDirFlt.txt', 'w+') as opt:
        for eachline in store_final_line_list:
            opt.write(eachline + '\n')

    store_final_line_list = []
    count = 0
    with open (opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion_addDirFlt.txt','r') as ipt:
        for eachline in ipt:
            eachline = eachline.strip('\n')
            col = eachline.strip().split()
            count += 1
            spe2_small_region = col[2]
            ...
            if count == 1:
                ...
                if col[7] != 'none':
                    syntenic_region_list = col[7].split(',')
    ...
    with open(opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion_addDirFlt_addRegFlt.txt', 'w+') as opt:
        for eachline in store_final_line_list:
            opt.write(eachline + '\n')
    ...
    with open (input_spe2_syntenic_genes_all_os_ACRs_fl,'r') as ipt:
        for eachline in ipt:
            ...
            if spe2_prefix == 'Sb':
                if re.match('.+_pg\d+\..+', col[3]):
                    mt = re.match('(.+_pg\d+)\..+', col[3])
                    geneID = mt.group(1)
                else:
                    geneID = col[3]
    ...
    store_final_line_list = []
    count = 0
    with open(opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion_addDirFlt_addRegFlt.txt','r') as ipt:
        for eachline in ipt:
            eachline = eachline.strip('\n')
            col = eachline.strip().split()
            count += 1
    ...
    with open(opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion_addDirFlt_addRegFlt_addspe2DirFlt.txt', 'w+') as opt:
        for eachline in store_final_line_list:
            opt.write(eachline + '\n')
    ...
    store_final_line_list = []
    count = 0
    with open (opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion_addDirFlt_addRegFlt_addspe2DirFlt.txt','r') as ipt:
        for eachline in ipt:
            eachline = eachline.strip('\n')
            col = eachline.strip().split()
            count += 1
    ...
    with open (opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion_addDirFlt_addRegFlt_addspe2DirFlt_addspe2CT.txt','w+') as opt:
        for eachline in store_final_line_list:
            opt.write(eachline + '\n')
...
def subfunction_add_species_specific_one_under_non_syntenic_regions (ipt_final_summary_fl,spe1_acr_fl,
    ...
    for eachacr in store_acr_not_in_summary_dic:
        ...
        final_line = 'none' + '\t' + eachacr + '\t' + \
                     'none' + '\t' + 'none' + '\t' + \
                     celltypecate_spe1 + '\t' + '\t'.join(none_list)
        store_final_line_list.append(final_line)

    with open (opt_dir + '/opt_' + spe1_prefix + '_' + spe2_prefix + '_addCelltype_SynRegion_addDirFlt_addRegFlt_addspe2DirFlt_addspe2CT_addSpeSpec.txt','w+') as opt:
        for eachline in store_final_line_list:
            opt.write(eachline + '\n')
