...
##read the file
ipt_soc_obj <- qs2::qs_read(ipt_output_soc_obj_fl)
...
plotUMAP <- function(b, ipt_output_dir,prefix="out", column="LouvainClusters_afthm", m1="umap1", m2="umap2",
  ...
  if (plot_bar == 'yes'){
    ...
    ##updating 113025
    b_filt <- b_filt[, c(2:(ncol(b_filt)-2), 1, (ncol(b_filt)-1):ncol(b_filt))]
...
final_meta_dt <- read.delim(paste0(ipt_output_dir,'/',ipt_prefix,".annotated_cell_identity.meta.txt"),header = T,row.names = 1)
final_obj <- append(ipt_soc_obj, list(
  final_meta = final_meta_dt
))

qs2::qs_save(final_obj,file=paste0(ipt_output_dir,'/',ipt_prefix,'.atac.soc.qs2'))
