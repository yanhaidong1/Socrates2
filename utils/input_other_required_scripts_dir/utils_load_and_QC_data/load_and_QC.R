...
##updating 020326 updating if no iscell, we will use the find cell to save the cells
...
if (open_build_object_final == 'no' & open_find_cells_step_final == 'no' & open_is_cells_step_final == 'no'){
  obj_find_cell <- qs2::qs_read(paste0(output_dir,'/',out,'.findcell_obj.qs2'))
  soc.obj <- convertSparseData(obj_find_cell, verbose=T)
  qs2::qs_save(soc.obj, file=paste0(output_dir,'/',out,".filtered.soc.qs2"))
}else{
  ...
  if (open_build_object_final == 'yes'){
    ...
    obj <- buildMetaData(obj,
                         ...
                         verbose=TRUE)
    qs2::qs_save(obj,paste0(output_dir,'/',out,'.raw_obj.qs2'))
    meta_fl <-obj$meta
  ...
  if (open_find_cells_step_final == 'yes'){
    ...
    if (open_build_object_final == 'yes'){
      ...
      obj <- qs2::qs_read(paste0(output_dir,'/',out,'.raw_obj.qs2'))
      #obj <- findCells(obj, 
      #                 output_dir,
      #                 doplot=T,
      #                 max.cells=16000,
    ...
    qs2::qs_save(obj,paste0(output_dir,'/',out,'.findcell_obj.qs2'))
  ...
  if (open_is_cells_step_final == 'yes'){
    ...
    if (open_find_cells_step_final == 'yes'){
      ...
      obj <- isCell(obj,output_dir,
                    ...
                    background.cutoff=min_tn5_val+500,
                    min.pTSS=0.2,
                    min.FRiP=0.2,
                    min.pTSS.z= -2,
                    min.FRiP.z= -2,
      ...
      obj <- qs2::qs_read(paste0(output_dir,'/',out,'.findcell_obj.qs2'))
      obj <- isCell(obj,output_dir,
                    num.test=num_test_val_final,
                    num.tn5=num_tn5_val_final,
                    num.ref=num_ref_val_final,
  ...
  # save QC data
  qs2::qs_save(obj, file=paste0(output_dir,'/',out, ".QC.qs2"))
  # convert to Socrates format for downstream analysis. 
  soc.obj <- convertSparseData(obj, verbose=T)
  
  ##check if the soc.obj contains the counts
  if (dim(soc.obj$counts)[2] == 0){
    obj_find_cell <- qs2::qs_read(paste0(output_dir,'/',out,'.findcell_obj.qs2'))
    soc.obj <- convertSparseData(obj_find_cell, verbose=T)
  }
