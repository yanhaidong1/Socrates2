...
loadData           <- function(input_soc_obj,
    ...
    # reformat sparse
    ##the updated activity is already the mtx
          ...
          if (target_cluster == 'LouvainClusters_afthm') {
            if ('LouvainClusters_afthm' %in% colnames(df)){
              ...
              if (target_cluster == 'LouvainClusters_sub') {
                ...
                if (target_cluster == 'Final_annotation_TCP_up') {
                  if ('Final_annotation_TCP_up' %in% colnames(df)){
...
normalize.activity <- function(output_dir,df, 
    ...
    qs2::qs_save(norm.act,paste0(output_dir,'/',output,".normalizedActivity_mtx.qs2"))
...
runMajorPriori     <- function(all.b, 
    ...
    # load data
    if(file.exists(paste0(output_dir,'/',output,".normalizedActivity_mtx.qs2"))){
      activity <- qs2::qs_read(paste0(output_dir,'/',output,".normalizedActivity_mtx.qs2"))
    ...
    ##updating 062221 
    if (onlysmooth_marker == 'yes'){
      qs2::qs_save(impute.activity,paste0(output_dir,'/opt_markers_impute.activity.qs2'))
    }else{
      qs2::qs_save(impute.activity,paste0(output_dir,'/opt_allgenes_impute.activity.qs2'))
    }
...
run_smooth     <- function(input_soc_obj,input_prefix,all.b, 
  ...
  qs2::qs_save(final_obj,file=paste0(output_dir,'/',input_prefix,'.atac.soc.qs2'))
