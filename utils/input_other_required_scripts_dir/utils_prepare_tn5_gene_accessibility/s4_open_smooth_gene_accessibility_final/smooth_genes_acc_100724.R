...
# load data
input_soc_obj <- qs2::qs_read(input_soc_obj_fl)

if(file.exists(paste0(output_dir,"/GAobj.qs2"))){
    dat <- qs2::qs_read(paste0(output_dir,"/GAobj.qs2"))
}else{
    dat <- loadData(input_soc_obj,target_cluster)
    ##updating 062221 save to the GAobj
    qs2::qs_save(dat,paste0(output_dir,"/GAobj.qs2"))
}
...
#qs2::qs_save(out,paste0(output_dir,'/out.qs2'))
