...
def update_prediction_by_cluster (input_clustering_fl,input_cell_prediction_fl,input_output_dir):
    ...
    with open (input_output_dir + '/opt_final_cell_annotation_per_cluster.txt','r') as ipt:
        for eachline in ipt:
            eachline = eachline.strip('\n')
            col = eachline.strip().split()
            cellID = col[1]
    ...
    with open (input_output_dir + '/opt_meta_ScAClass_predicted_cell_identity.txt','w+') as opt:
        for eachline in store_final_line_list:
            opt.write(eachline + '\n')
