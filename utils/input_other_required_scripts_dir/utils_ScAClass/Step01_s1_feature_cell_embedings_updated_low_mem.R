...
##updating 102124 we will reduce the feature and cells
...
##updating 102524 we will directly prepare the fts5000 
##updating 102124
##filter variable features but we will perform before the training data
#top.norm <- apply(training_dt, 1, var)
#top.norm.mean <- rowMeans(training_dt)
#norm.res <- residuals(lm(top.norm ~ top.norm.mean))
#norm.res <- norm.res[order(norm.res, decreasing = T)]
#top.n <- names(norm.res)[1:top_variant_ft_num]
#training_dt_top <- training_dt[top.n,]
#qs2::qs_save(training_dt_top,'./semroot_mtx_top5000fts.qs2')
...
#reference_obj <- FindVariableFeatures(object = reference_obj,nfeatures = 5000)
...
##udpating 102224
##we will split the testing_dt into different part
##for each part a max contain 2000 cells
total_cell_num = ncol(testing_dt)
range_size = 2000
start_points <- seq(1, total_cell_num, by = range_size)
end_points <- pmin(start_points + range_size - 1, total_cell_num)
ranges_df <- data.frame(start = start_points, end = end_points)

##build a new folder for each ranges df
total_row_num <- nrow(ranges_df)

for (i in 1:total_row_num){
  
  start_order = ranges_df[i,1]
  end_order = ranges_df[i,2]
  
  testing_sub_dt <- testing_dt[,start_order:end_order]
  
  opt_sub_dir <- paste0(input_output_dir,'/',start_order,'__',end_order)
  
  if (!dir.exists(opt_sub_dir)){
    dir.create(opt_sub_dir)
  } else {
    print("Dir already exists!")
  }
  
  
  rownames(testing_sub_dt) <- gsub('_','-',rownames(testing_sub_dt))
  shared_ID_test_ref <- intersect(rownames(testing_sub_dt),rownames(loadings_ref))
  ...
  testing_data <- as.matrix(testing_sub_dt[shared_ID_test_ref,])
  ...
  write.csv(test_embeddings_aligned_t,paste0(opt_sub_dir,'/opt_indep_testing_embeddings.csv'),quote = F)
