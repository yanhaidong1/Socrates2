##updating 063025 we will save the object
...
##updating 063025
ipt_soc_obj_fl <- as.character(args[4])
ipt_prefix <- as.character(args[5])


input_config_fl <- as.character(args[6])

input_output_dir <- as.character(args[7])
...
# Add new arguments
#parser$add_argument("--threshold", type="integer", required=TRUE, help="The threshold value for statistical testing.")
...
qs2::qs_save(results,'temp_results.qs2')
...
qs2::qs_save(null_dist_generation,'temp_null_dist_generation.qs2')
...
save_pval_null_dists <- paste0(input_output_dir,'/',prefix, ".combined_data.qs2")
qs2::qs_save(calculated_Pvals_all, file = save_pval_null_dists)
...
save_pval_null_dists <- paste0(input_output_dir,'/',prefix, ".all_pvalues.csv")
...
quantify_cell_type_specific_acrs <- function(ipt_soc_obj_fl,signifigant_matrix, pval_slot, pval_filter, prefix,ipt_prefix) {
        
  # Save histogram of p-values
    hist_save <- paste0(input_output_dir,'/',prefix, ".pvalue_dist.png")
    png(hist_save)
    ...
    write_delim(ct_specific_acrs_filtered_cell_types.filtered, file = paste0(input_output_dir,'/',prefix, ".pvalues.csv", collapse = "."), delim=",")
    ...
    write_tsv(cell_type_specific_acrs, file = paste0(input_output_dir,'/',prefix, ".all_cts.ACRs.bed", collapse = "."), col_names = FALSE)
    
    # Write Cell Type Specific Peaks by Cell Type 
    combined  %>% 
        group_by(cell_type)  %>% 
        dplyr::select(chrom, start, stop, sc_acr_name, pval)  %>% 
        group_walk(~ write_delim(.x, paste0(input_output_dir,'/',prefix,".", .y$cell_type, ".cts.ACRs.bed", collapse = "."), delim = "\t", col_names = FALSE))
    ...
    write_tsv(cell_type_restricted_acrs, file = paste0(input_output_dir,'/',prefix, ".all_ctr.ACRs.bed", collapse = "."), col_names = FALSE)
    ...
    write_tsv(all_acrs_combined, file = paste0(input_output_dir,'/',prefix, ".all_ACRs.classified.bed", collapse = "."), col_names = FALSE)
    
    ##updating 063025
    ##save to the object
    ipt_soc_obj <- qs2::qs_read(ipt_soc_obj_fl)
    final_obj <- append(ipt_soc_obj, list(
      diff_peak = all_acrs_combined
    ))
    qs2::qs_save(final_obj,file=paste0(input_output_dir,'/',ipt_prefix,'.atac.soc.qs2'))
...
if (stat_test == "perm") {
  quantify_cell_type_specific_acrs(ipt_soc_obj_fl,calculated_Pvals_all_quantify, "perm_pval", threshold, prefix,ipt_prefix)
} else if (stat_test == "pnorm") {
  quantify_cell_type_specific_acrs(ipt_soc_obj_fl,calculated_Pvals_all_quantify, "pnorm_pval", threshold, prefix,ipt_prefix)
} else {
