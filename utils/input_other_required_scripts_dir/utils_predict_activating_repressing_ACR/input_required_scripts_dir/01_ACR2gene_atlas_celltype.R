...
# #load arguments ----------------------------------------------------------------------------------
 args <- commandArgs(T)
#if(length(args)!=6){stop("Rscript ACR2gene_Liger_v3.2.R <scRNA_seurate.obj.qs2> <scATAC.soc.processed.qs2> <scatac.gene.sparse.qs2> <peak_sparse.qs2> <atac.meta> <prefix>")}
...
qs2::qs_save(final_obj,file=paste0(opt_dir,'/','temp','.ipt.qs2'))
