...
###################################################################################################
##updating 070425 this script we will use the object as the input
...
##updating 070425
ipt_soc_obj_fl <- as.character(args[2])
...
ipt_motif_rds_fl <- as.character(args[3])
...
input_BSgenome_config_fl <- as.character(args[5])
input_GCbias_config_fl <- as.character(args[6])
input_motifmatch_config_fl <- as.character(args[7])
...
#if(file.exists(paste0(output_dir,'/temp_peak_sparse_mtx.qs2'))){
  
#  message("peak sparse exists, directly load")
  
#  a <- qs2::qs_read(paste0(output_dir,'/temp_peak_sparse_mtx.qs2'))
...
a <- ipt_soc_obj$peak_tn5_mtx

#qs2::qs_save(a,paste0(output_dir,'/temp_peak_sparse_mtx.qs2'))
...
temp_peak_dt <- ipt_soc_obj$final_peak
...
qs2::qs_save(peaks,paste0(output_dir,'/peaks.qs2'))
qs2::qs_save(a, paste0(output_dir,'/a.qs2'))
...
qs2::qs_save(fragment_counts,paste0(output_dir,'/fragment_counts.qs2'))
...
qs2::qs_save(filtered_counts,paste0(output_dir,'/filtered_counts.qs2'))
...
jaspmotifs <- qs2::qs_read(ipt_motif_rds_fl)
...
#  qs2::qs_save(motif,paste0(output_dir,'/motif.qs2'))
#}
#if (species == 'rice_MSU') {
...
#motif            <- matchMotifs(jaspmotifs, filtered_counts, genome = BSgenome.Gmax.a4.v1)
qs2::qs_save(motif, file=paste0(output_dir,'/opt_motif_matches.qs2'))
##save the motif to the matrix
motif_mtx <- as.matrix(assays(motif)$motifMatches)
motif_mtx[motif_mtx = FALSE] <- 0
qs2::qs_save(motif_mtx,file=paste0(output_dir,'/opt_motif_matches_mtx.qs2'))
