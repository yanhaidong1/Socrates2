...
# save to the object
motif_deviation_dt <- read.table(motifs)

final_b <- t(b)

#head(motif_deviation_dt)
#head(final_b)
final_obj <- append(ipt_soc_obj, list(
  motif_dev = motif_deviation_dt,
  smooth_motif_dev = final_b
))

qs2::qs_save(final_obj,file=paste0(output_dir,'/',prefix,'.atac.soc.qs2'))
