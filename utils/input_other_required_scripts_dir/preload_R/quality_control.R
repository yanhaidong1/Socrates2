...
writeReadME <- function(fileID="QC_README"){
    ...
    writeLines(c("#########################################################################################################",
                 "# All intermediate steps are saved in the .qs2 file, which can be loaded in R and further analysed/partitioned:",
                 "",
                 "suppressWarnings(suppressMessages(library(GenomicRanges)))",
                 "suppressWarnings(suppressMessages(library(GenomicFeatures)))",
                 "",
                 'obj <- qs2::qs_read("youroutput.qs2")',
                 "",
...
#'
#' # specify input files
#' > input.files <- list.files(pattern="*.qs2", path="/path/to/rds/directory")
#' > input.files
#' [1] "rep1.qs2" "rep2.qs2"
#'
