...
config <- as.character(arg[8]) ##this config records the target clusters: since we have different cell types we will use this information
...
output_dir <- as.character(arg[10])

target_cluster_col <- as.character(arg[11]) ##LouvainClusters
target_cluster_color <- as.character(arg[12]) ##na if no color

##updating loading gene 010623
#open_load_all_gene_acc <- as.character(arg[13]) ##yes or no
ipt_target_gene_list_fl <- as.character(arg[13]) ##if no we will use na to represent

input_number_top_plot_ACR_peudo <- as.numeric(arg[14])

##updating 041623
open_equal_cellnum_diff_clusters <- as.character(arg[15])

##updating 071023
whether_open_provided_reduced_meta_fl <- as.character(arg[16])
...
##updating 051425
loadData_option_version   <- function(sm_fl, mt_fl,gn_fl, tf_fl, ma_fl, rd_fl, doL2=0,
  ...
  #rd <- read.table(rd)
  print(rd_fl)
  rd <- qs2::qs_read(rd_fl)$svd

  
  ##load the ACR
  if (sm_fl != 'na'){
    message(" - loading ACR ...")
    sm <- qs2::qs_read(sm_fl) ##sparse matrix of ACRs
...
#if (file.exists(paste0(output_dir,'/',prefix,".pseudotime_allgene.qs2"))){
#  obj <- qs2::qs_read(paste0(output_dir,'/',prefix,".pseudotime_allgene.qs2"))
#}else{
  ...
  qs2::qs_save(obj, file=paste0(output_dir,'/',prefix,".pseudotime_allgene.qs2"))
#}

#obj <- loadData_c(sparse,meta,svd,motif,doL2=0, traj)
#qs2::qs_save(obj, file=paste0(prefix,".pseudotime.qs2"))
